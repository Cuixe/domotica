...
class Socket(object):
    ...
    def __str__(self):
        return "{'id':'"+str(self.id)+"','owner_id':'"+str(self.owner_id)+"','name':'"+self.name+"', 'number':'"+str(self.number)+", 'pin':'"+str(self.rasp_pin)+",' status':'"+str(self.status)+"'}"
    ...
    def update(self):
        Socket.dataSource.execute(Socket.__UPDATE, self.owner_id, self.name, self.number, self.rasp_pin, self.status, self.id)
    ...
    def reload_sockets():
        Socket.sockets = []
        rows = Socket.dataSource.get_rows(query="SELECT id, owner_id, name, number, rapsPin, status FROM api_socket")
        for row in rows:
